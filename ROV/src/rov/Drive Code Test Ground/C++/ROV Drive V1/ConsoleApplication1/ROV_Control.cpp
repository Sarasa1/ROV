...
#define INPUT_DEADZONE  ( 0.15f * FLOAT(0x7FFF) )  // Default to 24% of the +/- 32767 range.   This is a reasonable default value but can be altered if needed.
...
void CheckDeadZone()
{
	...
	for (i = 0; i <= 3; i++)
	{
		if (abs(Controller[i].value) < INPUT_DEADZONE)
		{
...
void RecieveData(HANDLE hSerial)
{
	...
	int i = 0;
	char AcX[bufferSize], AcY[bufferSize], AcZ[bufferSize], Tmp[bufferSize], GyX[bufferSize], GyY[bufferSize];
