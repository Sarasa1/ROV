...
long RX, RY, LX, LY;
int motorRange = 200;
int gimbalXPos = 90, gimbalYPos = 90;
...
void loop()
{
  if(Serial)
  {
    if(ReadInValues() == 0)
    {
      ...
      sonarDistance = analogRead(A1)*100;
...
void WriteOutValues()
{
  ...
  String buffer[8];
    ...
    Serial.print(editForSend((ypr[0]* 180/M_PI), 4));
    Serial.print(editForSend((ypr[1]* 180/M_PI), 4));
    Serial.print(editForSend((ypr[2]* 180/M_PI), 4));
    Serial.print(editForSend(aaReal.x, 5));
    Serial.print(editForSend(aaReal.y, 5));
    Serial.print(editForSend(aaReal.z, 5));
    Serial.print(editForSend(sonarDistance, 3));
}
...
void LeapMove()
{
  if (palmPitch == 180 && palmYaw == 180 && palmRoll == 180)
  {
    wrist.write(64);
  }
  ...
  else
  {
    if (palmRoll < 0)
    {
      palmRoll = map(palmRoll, -90, -180, 0, 90);
    }
    
    else if (palmRoll > 0)
    {
      palmRoll = map(palmRoll, 180, 90, 90, 180);
    }
    palmRoll = constrain(palmRoll, 5, 175);
    wrist.write(palmRoll-60);
  }
  
  clawDistance = map(clawDistance, 20, 90, 20, 160);
  clawDistance = constrain(clawDistance, 35, 175);
  claw.write(clawDistance);
}
