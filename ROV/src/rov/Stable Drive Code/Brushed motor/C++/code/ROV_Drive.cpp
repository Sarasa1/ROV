...
int main()
{
	...
	HANDLE hSerial = CreateFile(L"COM5", GENERIC_WRITE | GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
...
void RecieveData(HANDLE hSerial)
{
	...
	DWORD bytesRead = 0;
		...
		ReadFile(hSerial, incomingBuffer, 10, &bytesRead, NULL);

		if (bytesRead > 0)
		{
			for (j = 0; j < bytesRead - 1; j++)
			{
				printf("%c", incomingBuffer[j]);
			}
			printf("\n");
